...
本次构建的新镜像名为：`styx-webui:prod-202606062122`

对应的镜像包文件：`styx-webui-prod-202606062122.tar.gz`（约 257MB）
...
# 本地构建和导出
export TARGET_PLATFORM=linux/amd64
export APP_IMAGE=styx-webui:prod-202606062122
...
scp deploy-artifacts/styx-webui-prod-202606062122.tar.gz root@YOUR_SERVER_IP:/opt/styx/

# 服务器端操作
cd /opt/styx
gunzip -c styx-webui-prod-202606062122.tar.gz | docker load
docker compose -f docker-compose.prod.yml exec postgres pg_dump -U styx -d styx > styx-backup-$(date +%Y%m%d%H%M).sql
docker compose -f docker-compose.prod.yml run --rm app pnpm db:migrate
docker compose -f docker-compose.prod.yml up -d app
docker compose -f docker-compose.prod.yml logs -f app
