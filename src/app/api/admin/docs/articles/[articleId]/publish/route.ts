  ...
  try {
    ...
    return runProtectedMutation(
      {
        ...
        operation: 'POST /api/admin/docs/articles/[articleId]/publish',
        actorType: 'admin',
      ...
      async () => {
        const article = await updateDocArticleStatus({ articleId: params.articleId, status: 'published' });
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json(
        { error: { code: 'validation_error', message: 'Doc publish request is invalid.', issues: error.issues } },
        { status: 400 },
      );
    }
